...
obj = (-x + y - 1.2)^2 + (-1.2*x + y - 1.4)^2 + (-1.4*x + y - 1.6)^2 + (-1.6*x + y - 1.8)^2 ;

% --- Bước 3: Không có ràng buộc ---
g = [];     % không có constraint
p = [];     % không có tham số
...
% --- Bước 7: Khai báo giá trị ban đầu ---
args = struct;
args.x0 = [0; 0];    % bắt đầu từ điểm (0,0)
args.lbx = [-inf; -inf];  % lower bound cho x và y
args.ubx = [inf; inf];    % upper bound cho x và y
...
% --- Bước 4: Tạo bài toán NLP ---
%nlp_prob = struct('f', obj, 'x', vars, 'g', g);
